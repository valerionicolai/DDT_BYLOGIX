    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Nuovo Progetto</h1>
            <p class="mt-2 text-gray-600">Crea un nuovo progetto per il tuo cliente</p>
          </div>
      ...
      <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
          ...
          <p class="mt-1 text-sm text-gray-600">
            Compila tutti i campi richiesti per creare il progetto
          </p>
        </div>
      ...
      >
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
          <div class="mt-3 text-center">
            ...
            <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">Progetto Creato!</h3>
            <div class="mt-2 px-7 py-3">
              <p class="text-sm text-gray-500">
                Il progetto è stato creato con successo. Vuoi crearne un altro o tornare alla lista?
              </p>
            </div>
...
const handleSubmit = async (formData) => {
  ...
  try {
    const response = await post('/api/projects', formData)
    ...
    error.value = err.response?.data?.message || 'Errore durante la creazione del progetto'
    addNotification({
      type: 'error',
      title: 'Errore',
      message: error.value
...
const viewProject = () => {
  if (createdProject.value) {
    // Per ora torniamo alla lista progetti, in futuro si potrà implementare una vista dettaglio
    router.push({ name: 'Projects' })
  }
}
